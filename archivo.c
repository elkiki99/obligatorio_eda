...
Archivo CrearArchivo(char * nombre)
{
	...
	a->vs = crear_versiones(); // Crea el árbol de versiones
...
// ordenadas por nivel jerárquico e indentadas según muestra el ejemplo publicado en la letra (cada nivel está indentado por un tabulador).
    ...
    //versiones_imprimir_niveles(a->vs);
...
// En caso que TipoRet sea ERROR, en error se debe cargar cuál es el mismo.
    ...
    //chequear numero de lineas no sea mayor a nroLinea + 1
    if( (int) nroLinea > cantidad_lineas(lineas(versiones_raiz(vs))) + 1) { // si tiene 3 lineas y ponemos 1, 2, 3 o 4 funciona, pero 5 ya no
        strcpy(error, "El número de líneas es incorrecto");
        return ERROR;
    }

    // Si está todo bien... creamos la linea y la insertamos
    insertar_linea(versiones_raiz(vs), linea, nroLinea); // función en versiones
    return OK;
